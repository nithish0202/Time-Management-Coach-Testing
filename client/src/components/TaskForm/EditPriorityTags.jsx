...
function EditPriorityTags({ open, onClose, task, onSave }) {
  ...
  const handleChange = (group) => (event) => {
    if (!isEditable) return; // Prevent changes if task is completed
    setTags(prev => ({
      ...prev,
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        {!isEditable && (
          <p style={{ color: 'red', marginBottom: 20 }}>
            This task is already completed. You cannot edit its tags.
          </p>
        )}
      ...
      <DialogActions>
        <Button onClick={onClose} style={{ background: "#a1a1a1", color: "white" }}>
          Cancel
        </Button>
        {isEditable && (
